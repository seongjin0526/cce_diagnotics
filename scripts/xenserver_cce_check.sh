...
check_CSAP_Xenserver_02() {
    local status="양호"
    local detail=""
    local cmd="cat /etc/passwd; grep /bin/bash /etc/passwd | cut -f1 -d:"
    local cur_state=""
...
check_CSAP_Xenserver_26() {
    local status="양호"
    local detail=""
    local cmd="Xsconsole → remote Service Configuration → Enable/Disable Remote Shell에서"
    local cur_state=""
...
check_CSAP_Xenserver_27() {
    local status="양호"
    local detail=""
    local cmd="Xen Master → Xenserver → network vlan 확인 → 인터뷰를 통해 분리되어서"
    local cur_state=""
...
check_ISMS_HV_05() {
    local status="양호"
    local detail=""
    local cmd="grep /bin/bash /etc/passwd | cut -f1 -d:; xe subject-list"
    local cur_state=""
...
check_ISMS_HV_14() {
    local status="양호"
    local detail=""
    local cmd="netstat -an | grep \"udp\" 또는 # netstat -an | grep \" udp\" | egrep \"514\""
    local cur_state=""
...
check_ISMS_HV_19() {
    local status="양호"
    local detail=""
    local cmd="xe vm-list name-label=\"<VM_NAME>\"; xe vbd-list vm-uuid=<VM_UUID>; xe vdi-list"
    local cur_state=""
...
check_ISMS_HV_23() {
    local status="양호"
    local detail=""
    local cmd="xe pif-list network-name-label=<네트워크 이름>; xe vif-list vm-name-label=<VM 이름>; xe pif-param-list uuid=<uuid_of_pif>"
    local cur_state=""
