...
check_CSAP_Apache_01() {
    local status="양호"
    local detail=""
    local cmd="cat [Apache 환경 설정 파일] | grep DocumentRoot"
    local cur_state=""
...
check_CSAP_Apache_02() {
    local status="양호"
    local detail=""
    local cmd="cd [Apache 설치 디렉터리]; find . -name manual 명령어"
    local cur_state=""
...
check_CSAP_Apache_03() {
    local status="양호"
    local detail=""
    local cmd="cat /etc/apache2/apache2.conf | grep FollowSymLinks; cat [Apahce Alias 설정 파일] | grep Alias"
    local cur_state=""
...
check_ISMS_WEB_06() {
    ...
    status="수동점검"
    detail="수동 점검 필요 항목입니다. 상위 디렉터리 접근 기능을 제거한 경우"
    cur_state="수동점검 필요"
...
check_ISMS_WEB_18() {
    local status="양호"
    local detail=""
    local cmd="cat /[Apache_Dir]/conf/httpd.conf(또는 apache2.conf)"
    local cur_state=""
...
check_ISMS_WEB_20() {
    local status="양호"
    local detail=""
    local cmd="apache2ctl -M | grep ssl"
    local cur_state=""
...
check_ISMS_WEB_24() {
    local status="양호"
    local detail=""
    local cmd="ls -al /[Apache 업로드 디렉터리]"
    local cur_state=""
    ...
    local target_spec_1
    target_spec_1=/apache2/apache2.conf
    ...
    local target_spec_4
    target_spec_4=/var/www/html/uploads/
...
check_ISMS_WEB_25() {
    local status="양호"
    local detail=""
    local cmd="/[Apache 설치 디렉터리]/httpd -v"
    local cur_state=""
...
check_ISMS_WEB_26() {
    ...
    status="수동점검"
    detail="수동 점검 필요 항목입니다. 로그 디렉터리 및 파일에 일반 사용자의 접근 권한이 없는 경우"
    cur_state="수동점검 필요"
